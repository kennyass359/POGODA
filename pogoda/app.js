        ...
        fetch(apiUrl)
            ...
            .then((data) => {
                if (data.main && data.main.temp) {
                    ...
                    // Проверим наличие coord в данных
                    if (data.coord && data.coord.lat && data.coord.lon) {
                        // Обновим карту с новыми координатами
                        updateMap(data.coord.lat, data.coord.lon);
                    } else {
                        console.error("Ошибка: Невозможно получить координаты из данных");
                    }
                } else {
                    console.error("Ошибка: Невозможно получить данные о температуре");
                }
            })
            .catch((error) => console.error("Ошибка при получении данных:", error));
            
    } // Закрываем функцию updateForecast
